...
class SQLBaseCollection(db_collection.DBCollection):
    ...
    def create_table(self):
        ...
        self._db.execute_ddl(clause, vals)
    ...
    def _fetch_one(self, clause, parats):
        curr = self._db.execute_sql(clause, parats)
        res = curr.fetchone() if curr.fetchone() else None
        curr.close()
        return res


    def _fetch_all(self, clause, params):
        curr = self._db.execute_sql(clause, params)
...
class SQLBaseDatabase(database.Database):
    ...
    def get_mannaged_collection(self, name, schema):
